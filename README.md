...
- **自動更新**: GitHub Actionsで3時間ごとに天気情報を更新
- **3日間天気予報**: 今日・明日・明後日の天気と気温を表示
- **48時間予報グラフ**: 3時間ごとの気温変化を折れ線グラフで視覚化
- **天気アイコン**: Unicode絵文字で天気を視覚的に表示 (☀️☁️☔など)
- **ニュースフィード**: NHKニュース(主要・経済)の最新5件ずつを表示
- **はてなブックマーク**: 人気エントリー(総合・学び)の最新5件ずつを表示
- **省電力**: JavaScriptなしで動作、Kindleのバッテリーを節約
- **自動リロード**: 30分ごとにページを自動更新
- **ダークモード**: ライト/ダークモード切り替え対応

## スクリーンショット

3日間の天気予報、48時間の気温予報グラフ、最新ニュースを1画面に表示。
...
3. ブックマークに追加
4. ページは30分ごとに自動リロードされます (サーバー側のデータは6時間ごとに更新)
...
- **認証**: 不要
- **データ**: 主要ニュース・経済ニュースの最新5件ずつ

### 3. はてなブックマーク RSS
- **提供元**: [はてなブックマーク](https://b.hatena.ne.jp/)
- **認証**: 不要
- **データ**: 人気エントリー(総合・学び)の最新5件ずつ

## 更新頻度

- **自動更新**: 3時間ごと (0, 3, 6, 9, 12, 15, 18, 21時 JST)
...
- **ホスティング**: GitHub Pages
- **外部API**:
  - 天気予報: weather.tsukumijima.net (3日間の予報)
  - ニュース: NHK RSS
  - はてなブックマーク: b.hatena.ne.jp
- **パフォーマンス最適化**:
  - HTTPクライアントの再利用 (Keep-Alive接続)
  - API呼び出しの並列化 (goroutine + channel)
